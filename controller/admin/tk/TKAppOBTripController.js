app.controller('TKAppOBTripController',[ '$scope', '$rootScope', '$location', '$window', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $window, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	$scope.app_ob_functions = function(){
		...
		vm.dtOptions = DTOptionsBuilder.newOptions()
			.withOption('ajax', {
				url: apiUrl+'admin/tk/app/ob/new_data.php',
				type: 'POST',
				data: function(d){
		...
		vm.dtColumns = [
			...
			DTColumnBuilder.newColumn(null).withTitle('Status').withClass('btnTDC').renderWith(function (data, type, full, meta) {
				...
				} else if (data.ob_status == "DECLINED") {
					btn = '<span style="font-weight:600 !important; color: red !important"> ' + data.ob_status + ' </span>';
				} else {
					btn = '<span style="font-weight:600 !important; color: #ffc000 !important"> ' + data.ob_status + ' </span>';
				}
				return btn;
			})
		];
		...
		$(document).ready(function () {
			...
			var str_div = '<div style="width: 100%; text-align: center; background: #1874CD; font-size: 22pt; color: white;">Official Business Trip</div>';
		...
		$scope.export = function () {
			...
			var _from		= typeof $scope.search_dfrom === "undefined" ? '' : $scope.search_dfrom  ;
			var _to			= typeof $scope.search_dto === "undefined" ? '' : $scope.search_dto  ;
			...
			var url = apiUrl+"admin/tk/app/ob/export.php?datenow=" + datenow + "&company=" + company + "&_from=" + _from + "&search_acct=" + search_acct + "&_to=" + _to ;
