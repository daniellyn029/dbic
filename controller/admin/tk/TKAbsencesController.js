app.controller('TKAbsencesController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	//Table Funtion
	$scope.absences_func = function (){
		var urlData = {
			'accountid'	: $scope.dashboard.values.accountid,
		}

		$http.post(apiUrl+'admin/tk/report/absences/view.php',urlData)
		.then( function (response, status){		
	...
	$scope.search_func= function(){
		...
		$http.post(apiUrl+"admin/tk/report/absences/search.php", urlData)
		.then( function (response, status){     
			var data = response.data;
			...
			if($scope.d_from == '' || $scope.d_from == null){
				$scope.filter += '';
			}else{
				if( $scope.filter != '' ){
					$scope.filter += ' And Date From: ' + $scope.d_from;
				}else{
					$scope.filter = 'Date From: ' + $scope.d_from;
				}
			}
			
			if($scope.d_to == '' || $scope.d_to == null){
				$scope.filter += '';
			}else{
				if( $scope.filter != '' ){
					$scope.filter += ' And Date To: ' + $scope.d_to;
				}else{
					$scope.filter = 'Date To: ' + $scope.d_to;
				}
			}
	...
	$scope.export = function () {
		...
		var d_from		= typeof $scope.d_from === "undefined" ? '' : $scope.d_from  ;
		var d_to		= typeof $scope.d_to === "undefined" ? '' : $scope.d_to  ;
		...
		var url = apiUrl+"admin/tk/report/absences/export.php?idsuperior=" + idsuperior + "&emp=" + emp + "&d_from=" + d_from + "&d_to=" + d_to + "&department=" + department + "&company=" + company + "&datenow=" + datenow;	
