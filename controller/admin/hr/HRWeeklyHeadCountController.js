app.controller('HRWeeklyHeadCountController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	//Search Function
	$scope.searchWeeklyHeadCount = function(){
		if($scope.dfrom==''||$scope.dfrom==null){
			$rootScope.dymodalstat = true;
			$rootScope.dymodaltitle= "Warning!";
			$rootScope.dymodalmsg  = "Please select date from";
		...
		if($scope.dto==''||$scope.dto==null){
			$rootScope.dymodalstat = true;
			$rootScope.dymodaltitle= "Warning!";
			$rootScope.dymodalmsg  = "Please select date to";
			$rootScope.dymodalstyle = "btn-warning";
			$rootScope.dymodalicon = "fa fa-exclamation-triangle";				
			$("#dymodal").modal("show");
			return;
		...
		if($scope.dfrom>$scope.dto ){
			$rootScope.dymodalstat = true;
			$rootScope.dymodaltitle= "Warning!";
			$rootScope.dymodalmsg  = "Date from must be less than date to";
			$rootScope.dymodalstyle = "btn-warning";
			$rootScope.dymodalicon = "fa fa-exclamation-triangle";				
			$("#dymodal").modal("show");
			return;
		...
		$http.post(apiUrl+"admin/hr/report/viewWeeklyHeadCount.php", urlData)
		.then( function (response, status){     
			var data = response.data;
			$scope.date = data[0].date;
			$scope.time = data[0].time;
			$scope.weeklyheadcountView = data;
			//filtered By function
			$scope.filter = $scope.dfrom+' to '+$scope.dto;
			$("#myModal").modal("hide");
		}, function(response) {
			$rootScope.modalDanger();
		}); 
	...
	$scope.exportReportToExcel = function (eleID) {
		...
		$http.post(apiUrl+'admin/hr/report/exportWeeklyHeadCount.php',urlData)
		.then( function (response, status){	
			var data = response.data;	
			if(data.status !='empty'){
				...
				var fileName = "HeadCountComparativeReport";			
				...
				if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)){
					...
					link.setAttribute("download", "HeadCountComparativeReport.csv");
					link.click();
