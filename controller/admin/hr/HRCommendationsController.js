app.controller('HRCommendationsController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	//Table Function
	$scope.viewCommendationsReport = function(){
		$scope.commendationsView = [];
		
		var urlData = {
			'accountid'	: $scope.dashboard.values.accountid,
		}
		$http.post(apiUrl+'admin/hr/report/viewCommendations.php',urlData)
		.then( function (response, status){		
	...
	//Search Function
	$scope.searchCommendations = function(){
		$scope.filter = '';
		var urlData = {
			'empid'			 		:$scope.emp,
			'department' 			:$scope.department,
			'emplyract'				:$scope.emplyract
		}
		$http.post(apiUrl+"admin/hr/report/searchCommendations.php", urlData)
		.then( function (response, status){     
			var data = response.data;
			$scope.commendationsView = data;
			...
			if($scope.emplyract == '' || $scope.emplyract == null){
				$scope.filter += '';
			}else{
				if($scope.filter!=''){
					$scope.filter += ' And Employer Action';
				}else{
					$scope.filter = 'Employer Action';
				}
			}
		}, function(response) {
			$rootScope.modalDanger();
	...
	$scope.exportReportToExcel = function (eleID) {
		...
		$http.post(apiUrl+'admin/hr/report/exportCommendations.php',urlData)
		.then( function (response, status){	
			var data = response.data;	
			if(data.status !='empty'){
				...
				var fileName = "CommendationsReport";			
				...
				if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)){
					...
					link.setAttribute("download", "CommendationsReport.csv");
					link.click();
