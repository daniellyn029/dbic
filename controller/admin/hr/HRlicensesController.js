app.controller('HRlicensesController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
    ...
    //Table Function
	$scope.viewLicensesReport = function(){
		...
		$http.post(apiUrl+'admin/hr/report/viewLicenses.php',urlData)
		.then( function (response, status){		
				
			var data 		= response.data;

			$scope.licensesDate = data[0].date;
			$scope.licensesTime = data[0].time;
	...
	$scope.searchLicense = function(){
		...
		var urlData = {
            ...
            'desig_licen'	:$scope.desig_licen,
            

		}
		$http.post(apiUrl+"admin/hr/report/searchLicense.php", urlData)
		.then( function (response, status){     
			var data = response.data;
			...
			if($scope.desig_licen == '' || $scope.desig_licen == null){
				$scope.filter += '';
			}else{
				if($scope.filter!=''){
					$scope.filter += ' And Designation License ';
				}else{
					$scope.filter = 'Designation License ';
				}
			}
   ...
   //EXPORT FUNCTION
	$scope.exportReportToExcel = function (eleID) {
		...
		$http.post(apiUrl+'admin/hr/report/exportLicense.php',urlData)
		.then( function (response, status){	
			var data = response.data;	
			if(data.status !='empty'){
				...
				//PAGE HEADER
				var page_header = '';
					page_header +=  document.getElementById('license_companyName').innerText + '\r\n' +
									document.getElementById('license_txt').innerText + '\r\n' +
									document.getElementById('license_filter').innerText + '\r\n' +
									document.getElementById('license_date').innerText + '\r\n';
				...
				if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)){
					...
					link.setAttribute("download", "LicenseReport.csv");
					link.click();
