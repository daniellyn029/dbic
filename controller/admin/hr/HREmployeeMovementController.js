app.controller('HREmployeeMovementController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	//Table Function
	$scope.viewEmpMovementReport = function(){
		$scope.empmovementView = [];
		
		var urlData = {
			'accountid'	: $scope.dashboard.values.accountid,
		}
		$http.post(apiUrl+'admin/hr/report/viewEmpMovement.php',urlData)
		.then( function (response, status){		
	...
	//Search Function
	$scope.searchEmpMovement = function(){
		$scope.filter = '';
		var urlData = {
			'empid'			 		:$scope.emp,
			'department' 			:$scope.department,
			'move_type'				:$scope.move_type,
		}
		$http.post(apiUrl+"admin/hr/report/searchEmpMovement.php", urlData)
		.then( function (response, status){     
			var data = response.data;
			$scope.empmovementView = data;
			...
			if($scope.move_type == '' || $scope.move_type == null){
				$scope.filter += '';
			}else{
				var move_type = $( "#move_type option:selected" ).text();
				if($scope.filter!=''){
					$scope.filter += ' And Movement Type '+move_type;
				}else{
					$scope.filter = 'Movement Type '+move_type;
				}
			}
		}, function(response) {
			$rootScope.modalDanger();
	...
	$scope.exportReportToExcel = function (eleID) {
		...
		$http.post(apiUrl+'admin/hr/report/exportEmpMovement.php',urlData)
		.then( function (response, status){	
			var data = response.data;	
			if(data.status !='empty'){
				...
				var fileName = "EmployeeMovementReport";			
				...
				if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)){
					...
					link.setAttribute("download", "EmployeeMovementReport.csv");
					link.click();
