app.controller('HRExaminationsSummaryController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	//Table Function
	$scope.viewExamSummaryReport = function(){
		$scope.examsummaryView = [];
		
		var urlData = {
			'accountid'	: $scope.dashboard.values.accountid,
		}
		$http.post(apiUrl+'admin/hr/report/viewExaminationsSummary.php',urlData)
		.then( function (response, status){		
	...
	//Search Function
	$scope.searchExaminationsSummary = function(){
		$scope.filter = '';
		var urlData = {
			'empid'			 		:$scope.emp,
			'department' 			:$scope.department,
			'examtaken'				:$scope.examtaken
		}
		$http.post(apiUrl+"admin/hr/report/searchExaminationsSummary.php", urlData)
		.then( function (response, status){     
			var data = response.data;
			$scope.examsummaryView = data;
			...
			if($scope.examtaken == '' || $scope.examtaken == null){
				$scope.filter += '';
			}else{
				if($scope.filter!=''){
					$scope.filter += ' And Exam Taken '+$scope.examtaken;
				}else{
					$scope.filter = 'Exam Taken '+$scope.examtaken;
				}
			}
		}, function(response) {
			$rootScope.modalDanger();
	...
	$scope.exportReportToExcel = function (eleID) {
		...
		$http.post(apiUrl+'admin/hr/report/exportExaminationsSummary.php',urlData)
		.then( function (response, status){	
			var data = response.data;	
			if(data.status !='empty'){
				...
				var fileName = "ExaminationsSummaryReport";			
				...
				if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)){
					...
					link.setAttribute("download", "ExaminationsSummaryReport.csv");
					link.click();
