app.controller('HRTrainingsTakenController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	//Table Function
	$scope.viewTrainingsTakenReport = function(){
		$scope.trainingsTaken = [];
		
		var urlData = {
			'accountid'	: $scope.dashboard.values.accountid,
		}
		$http.post(apiUrl+'admin/hr/report/viewTrainingsTaken.php',urlData)
		.then( function (response, status){		
	...
	//Search Function
	$scope.searchTrainingsTaken = function(){
		$scope.filter = '';
		var urlData = {
			'empid'			 		:$scope.emp,
			'department' 			:$scope.department,
			'trainingstaken'		:$scope.trainingstaken
		}
		$http.post(apiUrl+"admin/hr/report/searchTrainingsTaken.php", urlData)
		.then( function (response, status){     
			var data = response.data;
			$scope.trainingsTaken = data;
			...
			if($scope.trainingstaken == '' || $scope.trainingstaken == null){
				$scope.filter += '';
			}else{
				if($scope.filter!=''){
					$scope.filter += ' And Trainings Taken '+$scope.trainingstaken;
				}else{
					$scope.filter = 'Trainings Taken '+$scope.trainingstaken;
				}
			}
		}, function(response) {
			$rootScope.modalDanger();
	...
	$scope.reloadData = function(){
		...
		$scope.viewTrainingsTakenReport();
	...
	$scope.exportReportToExcel = function (eleID) {
		...
		$http.post(apiUrl+'admin/hr/report/exportTrainingsTaken.php',urlData)
		.then( function (response, status){	
			var data = response.data;	
			if(data.status !='empty'){
				...
				var fileName = "TrainingsTakenReport";			
				...
				if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)){
					...
					link.setAttribute("download", "TrainingsTakenReport.csv");
					link.click();
