app.controller('HRLateralTransferController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	
	//$cookieStore.remove('emailurl');

	$scope.headerTemplate="view/admin/header/index.html";
	$scope.leftNavigationTemplate="view/admin/hr/personnelaction/lateraltransfer/sidebar.html";
	...
	$scope.files2 = function(id){
		var file = id;
		window.location = '/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/pix/download.php?file='+file;
	}
	...
	$scope.currentTable = function(){
		...
		vm.dtOptions = DTOptionsBuilder.newOptions()
			.withOption('ajax', {
				url: apiUrl+'admin/hr/personnelaction/lateraltransfer/data.php',
				type: 'POST',
				data: function(d){
		...
		vm.dtColumns = [
            ...
            .renderWith(function(data, type, full, meta){
                    return '<button id="' + data.id + '" class="btn btn-flat btn-sm btn-success" data-target="#LateralTransferView" data-toggle="modal" onclick="angular.element(this).scope().getDataByIdCurrent(\'' +data.id+ '\')"><i class="fa fa-pencil-square-o fa-xs"></i> View</button>';
            }),
		];
		vm.dtInstance = {};
		...
		$scope.getDataByIdCurrent = function(id){
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/getDataByIdCurrent.php", urlData)
			.then(function(result){
				if(result.data.status == "empty"){
					...
					$scope.picFile = 'assets/php/admin/hr/personnelaction/lateraltransfer/pix/'+$scope.viewCurrent.picFile;
	...
	$scope.archiveTable = function(){
		...
		vm.dtOptions = DTOptionsBuilder.newOptions()
			.withOption('ajax', {
				url: apiUrl+'admin/hr/personnelaction/lateraltransfer/dataArchive.php',
				type: 'POST',
				data: function(d){
					d.accountid 	= $scope.dashboard.values.accountid,
		...
		vm.dtColumns = [
			...
			DTColumnBuilder.newColumn(null).withTitle('Action').notSortable().withClass('btnTD')
            .renderWith(function(data, type, full, meta){
                    return '<button class="btn btn-flat btn-sm btn-success" data-target="#LateralTransferViewArchive" data-toggle="modal" onclick="angular.element(this).scope().getDataByIdArchive(\'' +data.id+ '\')"><i class="fa fa-pencil-square-o fa-xs"></i> View</button>';
            }),
		];
		...
		$scope.getDataByIdArchive = function(id){
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/getDataByIdArchive.php", urlData)
			.then(function(result){
				if(result.data.status == "empty"){
					$scope.viewArchive = [];
				}else{
					$scope.viewArchive = result.data[0];
					$scope.picFileArchive = 'assets/php/admin/hr/personnelaction/lateraltransfer/pix/'+$scope.viewArchive.picFile;
	...
	$scope.approversetupTable = function(){
		...
		vm.dtOptions = DTOptionsBuilder.newOptions()
			.withOption('ajax', {
				url: apiUrl+'admin/hr/personnelaction/lateraltransfer/dataApproverSetup.php',
				type: 'POST',
				data: function(d){
					d.accountid = $scope.dashboard.values.accountid
				}
		...
		$scope.saveApproverSetup = function(){
            ...
            console.log(urlData);
            $http.post(apiUrl+'admin/hr/personnelaction/lateraltransfer/saveApproverSetup.php',urlData)
            .then( function (response, status){		
                var data = response.data;
        ...
        $scope.getApproverList = function(id){
            ...
            $http.post(apiUrl+'admin/hr/personnelaction/lateraltransfer/getApprovers.php',urlData)
            .then( function (response, status){
                $scope.isSaving = false;		
                var data = response.data;
	...
	$scope.autoFillEmpID = function(){
		...
		$http.post(apiUrl+'admin/hr/personnelaction/lateraltransfer/autoFillEmpID.php', urlData)
    ...
    $scope.showLateralTransferEntryModal = function(){
		//alert( $('.entryfrm').length );
		$scope.entry = [];
		var urlData = {
			'accountid': $scope.dashboard.values.accountid
		}
		$http.post(apiUrl+'admin/hr/personnelaction/lateraltransfer/newentryview.php',urlData)
		.then( function (response, status){		
			var data = response.data;
			$scope.entry = data;
			$scope.entry.actiontaken	= "Lateral Transfer";
	...
	$scope.autoFillEmpName = function(){
		...
		$http.post(apiUrl+'admin/hr/personnelaction/lateraltransfer/autoFillEmpName.php', urlData)
		.then(function(data, status){
	...
	$scope.updateEntryLateralTransfer= function( file ){
		...
		if($scope.viewCurrent.str_dept==$scope.viewCurrent.currentdeptname && $scope.viewCurrent.str_section==$scope.viewCurrent.currentsection){
			$rootScope.dymodalstat 	= true;
			$rootScope.dymodaltitle	= "Warning!";
			$rootScope.dymodalmsg  	= "Please select new department and section";
			$rootScope.dymodalstyle = "btn-warning";
			$rootScope.dymodalicon 	= "fa fa-exclamation-triangle";				
			$("#dymodal").modal("show");
			return;
		...
		$scope.isSaving = true;
		Upload.upload({
			url: apiUrl+'admin/hr/personnelaction/lateraltransfer/editEntry.php',
			method: 'POST',
			file: file,
			data: {
				'accountid'	: 	$scope.dashboard.values.accountid,
				...
				'targetPath': 	'../../../../admin/hr/personnelaction/lateraltransfer/pix/'						
			}
			...
			if( data.status == "error" ){
				...
				$("#LateralTransferView").modal("hide");
				$rootScope.dymodalstat = true;
	...
	$scope.saveEntryLateralTransfer= function( file ){
		...
		if($scope.entry.str_dept==$scope.entry.currentdeptname && $scope.entry.str_section==$scope.entry.currentsection){
			$rootScope.dymodalstat 	= true;
			$rootScope.dymodaltitle	= "Warning!";
			$rootScope.dymodalmsg  	= "Please select new department and section";
			$rootScope.dymodalstyle = "btn-warning";
		...
		$scope.isSaving = true;
		Upload.upload({
			url: apiUrl+'admin/hr/personnelaction/lateraltransfer/saveEntry.php',
			method: 'POST',
			file: file,
			data: {
				'accountid'	: 	$scope.dashboard.values.accountid,
			...
			if( data.status == "error" ){
				...
				$("#LateralTransferEntry").modal("hide");
				$rootScope.dymodalstat = true;
				$rootScope.dymodaltitle= "Success!";
	...
	$scope.editsection = function( id ){
		...
		$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/accounts.php", urlData)
		.then(function(result){
	...
	$scope.newsection = function( id ){
		...
		var obj = $filter('filter')($scope.esection, {id: id})[0];
		// console.log(obj);
		$scope.entry.newimmediatesupervisor= obj.idhead;
		$scope.supervisor = true;
		// $scope.entry.newimmediatesupervisor = "";
		
		// $scope.generateJobCodeByDept($scope.entry.newdeptname); //Jerald 13072020
	...
	$scope.generateManager = function( id ){
		...
		$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/manager.php", urlData)
		.then(function(result){
	...
	$scope.generateSection = function( id ){
		...
		$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/section.php", urlData)
		.then(function(result){
	...
	$scope.egeneratePosition = function( alias ){
		...
		$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/position.php", urlData)
		.then(function(result){
	...
	$scope.approve1 = function(){
		...
		if (r == true) {			
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/approve1.php", urlData)
			.then(function(result){
	...
	$scope.disapprove1 = function(){
		...
		if (r == true) {
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/disapprove1.php", urlData)
			.then(function(result){
				var data = result.data;
				if( data.status == "error" ){
					$rootScope.modalDanger();
	...
	$scope.approve2 = function(){
		...
		if (r == true) {
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/approve2.php", urlData)
			.then(function(result){
				var data = result.data;
				if( data.status == "error" ){
					$rootScope.modalDanger();
	...
	$scope.disapprove2 = function(){
		...
		if (r == true) {
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/disapprove2.php", urlData)
			.then(function(result){
				var data = result.data;
				if( data.status == "error" ){
					$rootScope.modalDanger();
	...
	$scope.approve3 = function(){
		...
		if (r == true) {
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/approve3.php", urlData)
			.then(function(result){
				var data = result.data;
				if( data.status == "error" ){
					$rootScope.modalDanger();
	...
	$scope.disapprove3 = function(){
		...
		if (r == true) {
			...
			$http.post("/dbic/assets/php/admin/hr/personnelaction/lateraltransfer/disapprove3.php", urlData)
			.then(function(result){
				var data = result.data;
				if( data.status == "error" ){
					$rootScope.modalDanger();
    ...
    $scope.entryModalTemplate="view/admin/hr/personnelaction/lateraltransfer/entry/index.html";
