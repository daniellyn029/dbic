app.controller('PositionController',[ '$scope', '$rootScope', '$location', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile','textAngularManager',
function($scope, $rootScope, $location, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager ){
	...
	$scope.position_functions = function(){
		$scope.add 		= [];
		$scope.edit		= [];
		$scope.search	= [];
		$scope.search.alias = ''; 
		...
		vm.dtOptions = DTOptionsBuilder.newOptions()
			.withOption('ajax', {
			 url: apiUrl+'admin/org/posi/data.php',
			 type: 'POST',
			 data: function(d){
		 ...
		 vm.dtColumns = [
			 ...
			 DTColumnBuilder.newColumn('name').withTitle('Position').notSortable().withClass('btnTD'),
		...
		$scope.resetCreatePos = function(){
			...
			$http.post(apiUrl+'admin/org/posi/add_view.php',urlData)
			.then( function (response, status){			
				var data = response.data;
				$scope.add = data;									
			}, function(response) {
		...
		$scope.edit_view = function( id ){
			...
			$http.post(apiUrl+'admin/org/posi/edit_view.php',urlData)
			.then( function (response, status){			
				var data = response.data;
				$scope.edit = data;									
			}, function(response) {
		...
		$scope.addUnitProcess = function(){
			...
			if (r == true) {
				...
				$http.post(apiUrl+'admin/org/posi/add_process.php',urlData)
				.then( function (response, status){		
					...
					if( data.status == "notloggedin" ){
						...
						$rootScope.dymodalstat = true;
						$rootScope.dymodaltitle= "Warning!";
						$rootScope.dymodalmsg  = "Please enter Position";
						$rootScope.dymodalstyle = "btn-warning";
						$rootScope.dymodalicon = "fa fa-exclamation-triangle";				
						$("#dymodal").modal("show");
						...
						$rootScope.dymodalmsg  = "New Position added";
		...
		$scope.editUnitProcess = function(){
			...
			if (r == true) {
				...
				$http.post(apiUrl+'admin/org/posi/edit_process.php',urlData)
				.then( function (response, status){	
					spinnerService.hide('form01spinner');
					$scope.isSaving=false;	
					var data = response.data;
					if( data.status == "notloggedin" ){
						...
						$rootScope.dymodalstat = true;
						$rootScope.dymodaltitle= "Success!";
						$rootScope.dymodalmsg  = "Position updated";
						$rootScope.dymodalstyle = "btn-info";
						$rootScope.dymodalicon = "fa fa-check";				
						$("#dymodal").modal("show");
					}
		...
		$scope.del_process = function( id ){
			...
			if (r == true) {
				...
				$http.post(apiUrl+'admin/org/posi/delete_process.php',urlData)
				.then( function (response, status){		
					spinnerService.hide('form01spinner');	
					var data = response.data;
					if( data.status == "notloggedin" ){
						...
						$rootScope.dymodalstat = true;
						$rootScope.dymodaltitle= "Success!";
						$rootScope.dymodalmsg  = "Position deleted";
						$rootScope.dymodalstyle = "btn-info";
						$rootScope.dymodalicon = "fa fa-check";				
						$("#dymodal").modal("show");
					}					
