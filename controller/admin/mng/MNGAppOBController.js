app.controller('MNGAppOBController', ['$scope', '$rootScope', '$location', '$window', '$routeParams', '$http', '$cookieStore', '$timeout', 'spinnerService', '$filter', 'Upload', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$compile', 'textAngularManager',
    function ($scope, $rootScope, $location, $window, $routeParams, $http, $cookieStore, $timeout, spinnerService, $filter, Upload, DTOptionsBuilder, DTColumnBuilder, $q, $compile, textAngularManager) {
        ...
        $scope.app_ob_functions = function () {
            ...
            vm.dtColumns = [
                DTColumnBuilder.newColumn('btn').withTitle('').notSortable().withClass('btnTD checkcircle'),
                ...
                DTColumnBuilder.newColumn(null).withTitle('Status').withClass('btnTD').renderWith(function (data, type, full, meta) {
                    ...
                    if (data.ob_status == "Approved") {
                        ...
                        btn = '<span style="font-weight:600 !important; color: gray !important"> ' + data.ob_status + ' </span>';
                    } else {
            ...
            $(document).ready(function () {
                ...
                var buttom = '<div> <button type="button" class="btn btn-primary" style="margin-right:5px;width:100px;border: 1px solid white;background:#0073b7" data-toggle="modal" data-target="#modal-approved" >Approve</button> <button type="button" class="btn btn-danger"  style="width:100px;border: 1px solid white;background:#0073b7" data-toggle="modal" data-target="#modal-disapproved">Disapprove</button> </div>';
            ...
            $scope.approve = function () {
                ...
                $http.post(apiUrl + 'admin/mng/ob/approve.php', urlData)
                    .then(function (response, status) {
                        $.notify('Official Business Trip Approved', "success");
            ...
            $scope.disapproved = function () {
                ...
                $http.post(apiUrl + 'admin/mng/ob/disapprove.php', urlData)
                    .then(function (response, status) {
                        $.notify('Official Business Trip Disapproved', "success");
