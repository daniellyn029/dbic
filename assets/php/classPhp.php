<?php
/*
Master Developer: Delsan Software Development Team
Contact Information: 09989954229
...
*********************************TRUNCATE QUERY STATEMENT*******************************/	
	...
	function getCompanyContactLink(){
		return "http://delsanonline.com/contact_us.html";
	}
	...
	function checkUnitName($con, $name, $id, $tbl ){
		$Qry = new Query();	
		$Qry->table     = $tbl;
		$Qry->selected  = "*";
	...
	function getAccounts($con, $id){
		...
		$Qry->selected  = "*";
		$Qry->fields    = "etypeid=1 ORDER BY empname ASC ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$data[] = array( 
	...
	function getAccountLeaves($con, $idacct){
		...
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				...
				if( !empty( $idacctleave ) || (int)$row['id'] <= 3 ){
					$data[] = array( 
						"id"        	=> $row['id'],
						"name"			=> $row['name'],
						"hours"			=> $row['hours'],
	...
	function getAccountLeaves2($con, $idacct){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "tblleaves AS a INNER JOIN tblleavestype AS b ON a.`idtype` = b.`id`";
		$Qry->selected  = "a.id, a.name, a.hours, a.idtype, b.type, a.isconvertible";
	...
	function getAccountLeaves3($con){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "tblleaves AS a INNER JOIN tblleavestype AS b ON a.`idtype` = b.`id`";
		$Qry->selected  = "a.id, a.name, a.hours, a.idtype, b.type, a.isconvertible";
	...
	function checkHolidayName($con, $name, $id, $date ){
		$Qry = new Query();	
		$Qry->table     = "tblholidays";
		$Qry->selected  = "*";
		$Qry->fields    = "name = '".$name."' AND date = '".$date."'";
		if( !empty($id) ){
			$Qry->fields= $Qry->fields." AND id!='".$id."'";
		}
		$rs = $Qry->exe_SELECT($con);
	...
	function insertTimeKeepingRec2($con, $idcalendar, $idacct){
		...
		$Qry->selected="*";
		$Qry->fields="id > '".$period['id']."' ORDER BY id ASC LIMIT 1 ";
		$rs=$Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>=1){
			if($row=mysqli_fetch_array($rs)){
				$date_eff = $row['period_start'];
			}
		}
		$Qry2 			= new Query();	
		$Qry2->table	= "tbltimesched";
		$Qry2->selected = "idacct, idsun, idmon, idtue, idwed, idthu, idfri, idsat, date_create";
		$Qry2->fields 	= "'".$idacct."', '".$calendar['idsun']."', '".$calendar['idmon']."', '".$calendar['idtue']."', '".$calendar['idwed']."', '".$calendar['idthu']."', '".$calendar['idfri']."', '".$calendar['idsat']."','".$date_eff."'";
	...
	function updateTimesheetLate($con, $param, $updateShift=NULL){
		...
		$shift_cols = array("monday"	=>"idmon,mon,mon_in,mon_out,mon_brkin,mon_brkout,mon_hrs", 
							"tuesday"	=>"idtue,tue,tue_in,tue_out,tue_brkin,tue_brkout,tue_hrs",
							"wednesday"	=>"idwed,wed,wed_in,wed_out,wed_brkin,wed_brkout,wed_hrs",
							"thursday"	=>"idthu,thu,thu_in,thu_out,thu_brkin,thu_brkout,thu_hrs",
							"friday"	=>"idfri,fri,fri_in,fri_out,fri_brkin,fri_brkout,fri_hrs",
							"saturday"	=>"idsat,sat,sat_in,sat_out,sat_brkin,sat_brkout,sat_hrs", 
							"sunday"	=>"idsun,sun,sun_in,sun_out,sun_brkin,sun_brkout,sun_hrs");
	...
	function getShiftIDData($con, $idshift){
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_array($rs)){
				$data = array($row['id'],$row['name'],$row['stime'],$row['ftime'],$row['breakin'],$row['breakout'],$row['reghrs']);
			}
		}
		return $data;
	}
	...
	function getBirthDates( $con, $date_s ){
		...
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$path = 'assets/images/undefined.jpg';
				if( !empty( $row['pic'] ) ){
	...
	function updateaccountjob1( $con, $data ){
		...
		$Qrye->exe_UPDATE($con);
		if($Qrye){
			foreach($data['allowance'] as $key => $value){
				// Update table
				$Qry2            = new Query();
				$Qry2->table     = "tblacctallowance";
				$Qry2->selected  = "amt='".$data['allowance'][$key]['new_amt']."'";
				$Qry2->fields 	 = "idallowance = '".$data['allowance'][$key]['idallowance']."'";
	...
	function getSalutation($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctsalutation";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getSuffix($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctsuffix";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getReligion($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctreligion";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getCitizenship($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctcitizenship";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getBloodtype($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctbloodtype";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getEyecolor($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblaccteyecolor";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getHaircolor($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblaccthaircolor";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getSkincolor($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctskincolor";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getBuildtype($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctbuildtype";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getDocumnets($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctdoctype";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getDocumentName( $con, $id ){
		$Qry 			= new Query();	
		$Qry->table     = "tblacctdoctype";
		$Qry->selected  = "alias";
		$Qry->fields    = "id=".$id;
	...
	function needtoAdd($con, $iddoc,  $idacct){
		$Qry 			= new Query();	
		$Qry->table     = "tblaccountdoc";
		$Qry->selected  = "id";
		$Qry->fields    = "idacct='".$idacct."' AND iddoc='".$iddoc."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			return false;
		}
	...
	function getPaytype($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblpaytype";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getGovPaytype($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblpaygovtype";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getAllowanceMethod($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblallowancetype";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getAccountAllowance( $con ){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblallowance";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getAccountAllowance1( $con, $idacct ){
		$data			= array();
		$Qry 			= new Query();	
	...
	function getUploaderTemplate($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tbluploader";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
	...
	function getAllowance($con, $idacct, $refno){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblformsallowance";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND idacct='".$idacct."' AND refno='".$refno."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$data[] = array(
					...
					"idallowance"	=>  $row['idallowance'],
					"new_amt"		=>  $row['new_amt'] ? number_format($row['new_amt'],2) : '0.00',
					"current_amt"	=>  $row['current_amt'] ? number_format($row['current_amt'],2) : '0.00',
				);
			}
		}
		return $data;
	}
	function getAllowancePersonnelAction($con, $idacct){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblacctallowance LEFT JOIN tblallowance ON tblacctallowance.idallowance = tblallowance.id";
		$Qry->selected  = "tblacctallowance.*,tblallowance.description";
		$Qry->fields    = "tblacctallowance.id>0 AND tblacctallowance.idacct='".$idacct."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$data[] = array(
	...
	function hasTimeSheetShift($con, $date, $idacct){
		$Qry = new Query();	
		$Qry->table ="vw_data_timesheet as a";
		$Qry->selected ="a.idshift";
		$Qry->fields =" a.idacct='".$idacct."' AND a.work_date = '".$date."' ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>=1){
			if($row=mysqli_fetch_array($rs)){
				return $row["idshift"];
	...
	function getMinMax($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "vw_datacurrentworkdates2";
		$Qry->selected  = "MIN(work_date) as minn, MAX(work_date) as maxx";
		$Qry->fields    = "work_date is not null";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$min_arr = explode("-",$row['minn']);
				$max_arr = explode("-",$row['maxx']);
				
				$years = array();
				for($x=$min_arr[0]; $x <= $max_arr[0]; $x++){
					$years[] = array(
						"year" => $x
					);
				}
				...
				$data = array(
					"years"		=>  $years,
					"months"	=> array(
						array("id"=>"01","month"=>"January"),
						array("id"=>"02","month"=>"February"),
						array("id"=>"03","month"=>"March"),
						array("id"=>"04","month"=>"April"),
						array("id"=>"05","month"=>"May"),
						array("id"=>"06","month"=>"June"),
						array("id"=>"07","month"=>"July"),
						array("id"=>"08","month"=>"August"),
						array("id"=>"09","month"=>"September"),
						array("id"=>"10","month"=>"October"),
						array("id"=>"11","month"=>"November"),
						array("id"=>"12","month"=>"December")
					)
				);
			}
		}
	...
	function getDeputy1($con, $idacct){
		$data			= '';
		$Qry 			= new Query();	
		$Qry->table     = "tblbunits";
		$Qry->selected  = "*";
	...
	function getPromotionApprover($con, $idacct){
		$data			= 0 ;
		$Qry 			= new Query();	
		$Qry->table     = "tblformsetup";
		$Qry->selected  = "*";
		$Qry->fields    = "(approver_1a = '".$idacct."' OR
							approver_1b = '".$idacct."' OR
							approver_2a = '".$idacct."' OR 
							approver_2b = '".$idacct."' OR
							approver_3a = '".$idacct."' OR
							approver_3b = '".$idacct."') AND idform = 2";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
	...
	function getDeputy2($con, $idacct){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblappmatrix";
		$Qry->selected  = "*";
		$Qry->fields    = " ((approver_1b = '".$idacct."') OR (approver_1c = '".$idacct."')) OR 
							((approver_2a = '".$idacct."') OR (approver_2b = '".$idacct."') OR (approver_2c = '".$idacct."')) " ;
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$arr_unit = getapprover( $con, $row['idunit'] );
				if( !empty($arr_unit['idhead']) && !in_array($arr_unit['idhead'],$data) ){
					array_push($data, $arr_unit['idhead']);
				}
			}
		}
		$Qry2 			= new Query();	
		$Qry2->table     = "tblbunits";
		$Qry2->selected  = "*";
		$Qry2->fields    = "idhead='".$idacct."'" ;
		$rs2 			= $Qry2->exe_SELECT($con);
		if(mysqli_num_rows($rs2)>= 1){
			while($row2=mysqli_fetch_array($rs2)){
				...
				$Qry3->selected  = "*";
				$Qry3->fields    = "( approver_type_2a = '1' AND approver_unit_2a = '".$idunit."' )" ;
				$rs3 			= $Qry3->exe_SELECT($con);
				if(mysqli_num_rows($rs3)>= 1){
					while($row3=mysqli_fetch_array($rs3)){
						if( !in_array($idacct,$data) ){
							array_push($data, $idacct);
						}
					}
				}
			}
	...
	function getLateralTransferApprover($con, $idacct){
		$data = 0 ;
		$Qry = new Query();
		$Qry->table = "tblformsetup";
		$Qry->selected = "*";
		...
		approver_2b = '".$idacct."' OR
		approver_3a = '".$idacct."' OR
		approver_3b = '".$idacct."') AND idform = 1";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$data = 1;
	...
	function getWageIncreaseApprover($con, $idacct){
		$data = 0 ;
		$Qry = new Query();
		$Qry->table = "tblformsetup";
		$Qry->selected = "*";
		...
		approver_2b = '".$idacct."' OR
		approver_3a = '".$idacct."' OR
		approver_3b = '".$idacct."') AND idform = 3";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$data = 1;
	...
	function getEmpAcctDept($con, $acct){
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "idunit";
		$Qry->fields    = "empid='".$acct."'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_array($rs)){					
				return $row['idunit'];
	...
	function getEmpAcctID($con, $acct){
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "id";
		$Qry->fields    = "empid='".$acct."'";
	...
	function checkTimelogPairRec( $con, $idacct, $work_date, $batchid, $machid ){
		$Qry = new Query();	
		$Qry->table     = "tbltimelogpair";
		$Qry->selected  = "id";
		$Qry->fields    = "idacct = '".$idacct."' AND work_date = '".$work_date."' AND batchid='".$batchid."' AND machid='".$machid."'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			return true;
		}
	...
	function getTimelogPairRec( $con, $idacct, $work_date ){
		$Qry = new Query();	
		$Qry->table     = "vw_dataloglink";
		$Qry->selected  = "*";
		$Qry->fields    = "idacct = '".$idacct."' AND work_date = '".$work_date."'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_array($rs)){			
				return array(
	...
	function getTimeLogsConf($con, $id=NULL){
		$data= array();
		$Qry = new Query();	
	...
	function checkConfigName($con, $name, $id ){
		$Qry = new Query();	
		$Qry->table     = "tbltimelogsconf";
		$Qry->selected  = "*";
		$Qry->fields    = "alias = '".$name."'";
	...
	function checkbatchid($con,$batchid,$machid){
		$Qry = new Query();	
		$Qry->table     = "tbltimelogpair";
		$Qry->selected  = "id";
		$Qry->fields    = "batchid='".$batchid."' AND machid='".$machid."'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			return true;
		}
	...
	function recExistsPrevDay( $con, $idacct, $work_date, $date_out, $time_out ){
		$Qry = new Query();	
		$Qry->table     = "vw_dataloglink";
		$Qry->selected  = "id";
		$Qry->fields    = "idacct='".$idacct."' AND work_date='".$work_date."' AND ((date_out='".$date_out."' AND time_out='".$time_out."') OR (date_out2='".$date_out."' AND time_out2='".$time_out."'))";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			return true;
		}
	...
	function checkLeaveType($con, $idleave, $idacct ){
		$Qry = new Query();
		$Qry->table = "tblaccountleaves";
		$Qry->selected = "*";
		$Qry->fields = "idacct='".$idacct."' AND idleave = '".$idleave."'";
	...
	function checkIfHoliday($con,$date){		
		$Qry = new Query();
		$Qry->table = "vw_dataholidays";
		$Qry->selected = "*";
		$Qry->fields = "date='".$date."'";
