...
$Qry->selected  = "dt.temp,de.id,de.sexstr,de.idunit,dt.idleave,dt.leavestat, dt.work_date, de.pic, de.empname, de.post, dt.shift_status, dt.in, dt.out,dt.shiftin,dt.shiftout,(SELECT GROUP_CONCAT(reso_txt) FROM vw_resocenter WHERE idacct = de.id AND reso_date = dt.work_date) AS reso";
$Qry->fields    = "de.id != '".$param->accountid."' AND /*dt.shift_status IS NOT NULL AND*/ dt.work_date = '".$date."' AND (de.idunit IN (".$ids.") or de.idsuperior='".$param->accountid."') ";
