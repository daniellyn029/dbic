        ...
        foreach ($value as $key2 => $value2)
        {
            ...
            if ($newsched)
            {
                ...
                else
                {
                    ...
                    $Qrycheck->selected = "*";
                    $Qrycheck->fields = "idacct='" . $value2->id . "' AND date='" . $value2->work_date . "' ORDER BY id DESC LIMIT 1";
                    ...
                    if (mysqli_num_rows($rscheck) >= 1)
                    {
                        while ($rowcheck = mysqli_fetch_array($rscheck))
                        {
                            ...
                            else
                            {
                                ...
                                $Qryb->fields = "idacct='" . $value2->id . "' AND date='" . $value2->work_date . "' ORDER BY id DESC LIMIT 1";
            ...
            else
            {
                $Qry = new Query();
                $Qry->table = "tbldutyroster";
                $Qry->fields = "idacct='" . $value2->id . "' AND date='" . $value2->work_date . "' AND secretary='0' AND type_creator='1'";
                $Qry->exe_DELETE($con);
            }
        }
    }
    ...
    if ($count != 0)
    {
        foreach ($param->employee as $key => $value)
        {
            foreach ($value as $key2 => $value2)
            {
                $Qryb = new Query();
                $Qryb->table = "tbldutyroster";
                $Qryb->selected = "manager='1', 
                                        date_approve ='" . SysDate() . "', 
                                        time_approve='" . SysTime() . "'";
                $Qryb->fields = "idacct='" . $value2->id . "' AND date='" . $value2->work_date . "' AND (secretary=1 OR secretary is null) AND (manager=0 OR manager IS NULL)";
                $checkentryb = $Qryb->exe_UPDATE($con);

                $timesheet_data = getTimeSheetData($con, $value2->id, $value2->work_date);
                if (!empty($timesheet_data))
                {
                    if (addToBackuptimesheet($con, $timesheet_data))
                    {
                        error_reporting(0);
                        $param
                            ->info->date = $value2->work_date;
                        $param
                            ->info->idacct = $value2->id;
