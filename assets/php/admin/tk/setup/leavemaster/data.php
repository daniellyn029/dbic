...
if(mysqli_num_rows($rs)>= 1){
    ...
    while($row=mysqli_fetch_array($rs)){
        if($row['type'] == 'Paid'){
             $pdleave = '<input type="checkbox" name="pdleave'.$row['id'].'[]" checked>';
        }
        if($row['type'] == 'Unpaid'){
            $pdleave = '<input type="checkbox" name="pdleave'.$row['id'].'[]">';
        }

        if($row['isconvertible'] == 'Y'){
            $isconvertible = '<input type="checkbox" name="isconvertible'.$row['id'].'[]" checked>';
       }
       if($row['isconvertible'] == 'N'){
           $isconvertible = '<input type="checkbox" name="isconvertible'.$row['id'].'[]">';
       }
