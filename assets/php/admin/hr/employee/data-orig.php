...
if( !empty( $param['empname'] ) ){ $search.=" AND empname like   '%".$param['empname']."%' "; }
...
}elseif( (int)$param["order"][0]["column"] == 0 ){
	$search=$search." ORDER BY empid ASC ";
}
...
if(mysqli_num_rows($rs)>= 1){
    ...
    while($row=mysqli_fetch_array($rs)){
		...
		if( !empty( $row['pic'] ) ){
			$path = 'assets/php/admin/hr/employee/pix/'.$row['pic'];
		}
