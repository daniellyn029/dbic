																	...
																	if( !empty( $param['info']['idtax'] ) ){																	
																		...
																		if( (int)$param['info']['empstat'] == 5 ){
																			...
																			foreach( $param['info']['leaves'] as $keys=>$val ){
																				...
																				}else if( in_array($val['id'],$unique_lv) ){
																		...
																		if( (int)$param['info']['empstat'] != 5 && (int)$param['info']['empstat'] != 6 && (int)$param['info']['orig_empstat'] == 5 ){
																			$return = json_encode(array("status"=>"noupdate"));
																			print $return;	
																			mysqli_close($con);
																			return;
																		}
																		if( (int)$param['info']['empstat'] != 5 && (int)$param['info']['empstat'] != 6 && !empty( $param['info']['rdate'] ) && strlen($param['info']['rdate']) > 1 ){
																			$return = json_encode(array("status"=>"noupdate"));
																			print $return;	
																			mysqli_close($con);
																			return;
																		...
																		if($checke){
																			...
																			foreach($param['info']['pa'] as $key => $value ) {
																				...
																				if( !empty($equi_tools) && !empty($serial) && !empty($quantity) && !empty($date_issued) ){
																					...
																					if( empty( $pa_id ) ){
																						$Qry->selected 	= "idacct,equi_tools,serial,quantity,date_issued";
																						$Qry->fields 	= "'".$last_id."','".$equi_tools."','".$serial."','".$quantity."','".$date_issued."'";
																					}else{
																						$Qry->selected 	= "equi_tools='".$equi_tools."',serial='".$serial."',quantity='".$quantity."',date_issued='".$date_issued."'";
																					}
...
function insertInfo($con, $param){
	...
	$Qry->selected 	= "idregion,basicpay_type,absences_type,lates_type,undertime_type,idcba,sss_type,ibig_type,health_type,sss_amt,ibig_amt,health_amt,
					   ...
					   customer,product,idacct,idpos,idunit,idlabor,idsuperior,empstat,idloc,wshift,schedtype,hdate,salary,idpaygrp,paystat,idrevenue,dependent,idrelease,idlvl,idlvlscale,riceallowance,clothingallowance,laundryallowance";
	$Qry->fields 	= "'".(int)$param['info']['idregion']."','".(int)$param['info']['basicpay']."', '".(int)$param['info']['absences']."', '".(int)$param['info']['lates']."', '".(int)$param['info']['undertime']."',
		...
		foreach( $param['info']['acctallowance'] as $keyss=>$valss ){
			if( !empty( $valss["idallowance"]) && !in_array($valss['idallowance'],$unique_al) ) {
				...
				if( empty( $valss['id'] ) ){
					...
					$Qry3->selected = "idacct='".$idacct."', idallowance='".$idallowance."', idmethod='".$idmethod."', amt='".$amt."'";
