...
require_once('../../../../email/emailFunction.php');
    ...
    if(!empty($param['accountid'])){
		...
		if( !chkpendingForm( $con, "tblforms03", $param['entry']['idacct'] ) ){
			$latest_approved = getLatestRequestPromotion( $con, "tblforms03", $param['entry']['idacct'] );
					...
					if( $param['entry']['newdeptname'] == $latest_approved['newiddept'] ){
						if( $param['entry']['currentsection'] == $latest_approved['currentsection'] ){
							if( $param['entry']['newsection'] == $latest_approved['newidsection'] ){
								if( $param['entry']['currentbasepay'] == $latest_approved['currentbasepay'] ){
									if( $param['entry']['newbasepay'] == $latest_approved['newbasepay'] ){
										if(count($latest_approved['allowance'])==count($param['entry']['allowance'])){
											$counter = 0;
											foreach ($param['entry']['allowance'] as $kkk => $vvv) {
												if($param['entry']['allowance'][$kkk]['new_amount'] != $latest_approved['allowance'][$kkk]['new_amt']){
													$counter++;
												}
											}
											if($counter==0){
        ...
        $linkid1 		=	$linkid + 1;
        $ticketNumber 	=	"PRO".str_pad($linkid1,6,"0",STR_PAD_LEFT);

        $Qry 			= new Query();	
        $Qry->table 	= "tblforms03";
        $Qry->selected 	= "requestor,refferenceno, empid, empname, effectivedate, empactiontaken, currentdeptname, newdeptname, newiddept, currentdeptmanager, newdeptmanager, newidmngr ,currentimmediatesupervisor, newimmediatesupervisor, newidsuperior, currentsection, newsection, newidsection, currentempstatus, newempstatus, currentjobcode, newjobcode, currentjoblevel, newjoblevel, currentpositiontitle, newpositiontitle, currentpaygroup, newpaygroup, currentlabortype, newlabortype, currentbasepay, newbasepay, currenttotalcashcomp, newtotalcashcomp, remarks, createdby, date_created, time_created, start_date, end_date, pay_date";
        $Qry->fields 	= " '".$param['entry']['idacct']."',
                            ...
                            '".$param['entry']['str_dept']."',
							'".$param['entry']['newdeptname']."',
                            '".$param['entry']['currentdeptmanager']."',
                            '".$param['entry']['str_mngr']."',
							'".$param['entry']['newdeptmanager']."',
                            '".$param['entry']['currentimmediatesupervisor']."',
                            '".$param['entry']['str_super']."',
							'".$param['entry']['newimmediatesupervisor']."',
                            '".$param['entry']['currentsection']."',
                            '".$param['entry']['str_section']."',
							'".$param['entry']['newsection']."',
                            '".$param['entry']['currentempstatus']."',
                            '".$param['entry']['newempstatus']."',
                            '".$param['entry']['currentjobcode']."',
                            '".$param['entry']['newjobcode']."',
                            '".$param['entry']['currentjoblevel']."',
                            '".$param['entry']['newjoblevel']."',
                            '".$param['entry']['currentpositiontitle']."',
                            '".$param['entry']['newpositiontitle']."',
                            '".$param['entry']['currentpaygroup']."',
                            '".$param['entry']['newpaygroup']."',
                            '".$param['entry']['currentlabortype']."',
							'".$param['entry']['newlabortype']."',
