...
*********************************TRUNCATE QUERY STATEMENT*******************************/	
    ...
    $Qry=new Query();
    $Qry->table="vw_data_promotion LEFT JOIN tblposition ON vw_data_promotion.newpositiontitle = tblposition.name LEFT JOIN tbljoblvl ON vw_data_promotion.newjoblevel = tbljoblvl.lvl LEFT JOIN tbllabortype ON vw_data_promotion.newlabortype = tbllabortype.type LEFT JOIN tblempstatus ON vw_data_promotion.newempstatus = tblempstatus.stat LEFT JOIN tblpaygrp ON vw_data_promotion.newpaygroup = tblpaygrp.group";
    $Qry->selected="vw_data_promotion.id,vw_data_promotion.idstatus,vw_data_promotion.effectivedate,vw_data_promotion.requestor,tblposition.id AS newpositiontitle,tbljoblvl.id AS newjoblevel,(CASE WHEN (vw_data_promotion.newidsection = '') THEN vw_data_promotion.newiddept ELSE vw_data_promotion.newidsection END) AS idunit,tbllabortype.id AS newlabortype,vw_data_promotion.newidsuperior, tblempstatus.id AS newempstatus,tblpaygrp.id AS newpaygroup,vw_data_promotion.newbasepay,vw_data_promotion.refferenceno";
    $Qry->fields="vw_data_promotion.idstatus=1 AND vw_data_promotion.effectivedate='".$date."' AND vw_data_promotion.201update=0";
    ...
    if(mysqli_num_rows($rs)>0){
        ...
        if($proceed==1){
			echo "*** AUTO UPDATE 201 PROMOTION ***\r\n";
