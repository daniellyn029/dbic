...
function getDocType($con, $accountid){
	$Qry=new Query(); 
	$data=array();
    $Qry->table="tblaccountdoc LEFT JOIN  tblacctdoctype ON tblaccountdoc.iddoc = tblacctdoctype.id LEFT JOIN tblaccount ON tblaccountdoc.idacct = tblaccount.id
	";
    $Qry->selected="tblaccountdoc.* , tblacctdoctype.document, tblaccount.empid";
    $Qry->fields="idacct='".$accountid."'";
    $rs=$Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>=1){
        while($row=mysqli_fetch_array($rs)){
...
function getCompensation($con, $accountid){
    ...
    $Qry->selected="a.*,b.salary,a.idpayroll,b.paygroup,b.basicpay_type,b.absences_type,b.daysmonth,b.lates_type,b.undertime_type,b.sss_type, b.ibig_type, b.health_type, b.sss_amt, b.ibig_amt, b.health_amt";
    $Qry->fields="idacct='".$accountid."'";
    $rs=$Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>=1){
        while($row=mysqli_fetch_array($rs)){
