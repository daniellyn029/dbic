...
if (mysqli_num_rows($rs) >= 1) {
    ...
    while ($row = mysqli_fetch_array($rs)) {
        ...
        if (!empty($row['holiday_id'])) {
            $row['shift_status'] = '<p style="color:red; font-weight: 900;"> '. ucwords(strtolower($row['holiday_type'])) . ' Holiday' .'</p>';
        }

        if ($row['holiday_idtype']==1) {
            $row['shift_status'] = '<p style="color:red; font-weight: 900;"> LH' .'</p>';
        }

        if ($row['holiday_idtype']==2) {
            $row['shift_status'] = '<p style="color:red; font-weight: 900;"> SH' .'</p>';
        }
        if ($row['holiday_idtype']==3) {
            $row['shift_status'] = '<p style="color:red; font-weight: 900;"> LSH' .'</p>';
        }
        ...
        if ($row['obid']) {
            if ($row['ob_stat'] == '3') {
                $pending++;
                $ob = '<div id="block_container"><div id="bloc1" class="dot3">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><div id="bloc2"><span>' . $row['ob_type'] . '<a href="" ng-click="cancelapplication(\'obtrip\', ' . $row['aaid'] . ')"><i class="fa fa-times danger" aria-hidden="true"></i> </a> </span></div></div>';
            }
            if ($row['ob_stat'] == '2') {
                $declined++;
                $ob = '<div id="block_container"><div id="bloc1" class="dot2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><div id="bloc2"><span>' . $row['ob_type']  . '</span></div></div>';
           
            }
            if ($row['ob_stat'] == '1') {
                $approved++;
                $ob = '<div id="block_container"><div id="bloc1" class="dot">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><div id="bloc2"><span>' . $row['ob_type'] . '</span></div></div>';
        ...
        if ($row['idleave']) {
            ...
            if ($row['leave_status'] == 'DECLINED') {
                $lv = '<div id="block_container"><div id="bloc1" class="dot2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><div id="bloc2"><span>' . $row['leavename'] . ' - ' . $row['leavehrs'] . 'hrs </span></div></div>';
            
                $declined++;
            }
            if ($row['leave_status'] == 'APPROVED') {
                $lv = '<div id="block_container"><div id="bloc1" class="dot">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><div id="bloc2"><span>' . $row['leavename'] . ' - ' . $row['leavehrs'] . 'hrs </span></div></div>';
        ...
        // if ($row['shift_status'] == 'Special Holiday') {
        //     $row['shift_status'] = '<p class="cdanger fw9">SH</p>';
        // }

        // $totalwh        = $totalwh + $twh;
        $totalwh        = $totalwh + $row['reghrs'];
        // $totalewh       = $totalewh + $others;
        $totalewh       = $totalewh + sprintf('%0.2f', $row['excess']);
